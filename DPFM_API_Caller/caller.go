...
import (
	...
	"data-platform-api-freight-agreement-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) headerDelete(
	...
	header.FreightAgreement = input.Header.FreightAgreement
	header.IsMarkedForDeletion = input.Header.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "FreightAgreementHeader", "runtime_session_id": sessionID})
	if err != nil {
...
func (c *DPFMAPICaller) itemDelete(
	...
	sessionID := input.RuntimeSessionID
	item := c.ItemRead(input, log)
	item.FreightAgreement = input.Item.FreightAgreement
	item.FreightAgreementItem = input.Item.FreightAgreementItem
	item.IsMarkedForDeletion = input.Item.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": item, "function": "FreightAgreementItem", "runtime_session_id": sessionID})
	if err != nil {
		err = xerrors.Errorf("rmq error: %w", err)
		log.Error("%+v", err)
		return nil
